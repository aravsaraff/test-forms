  ...
  async componentDidMount() {
    ...
    if (resp.status === 200) {
      if (resp.data.message) {
        ...
        // console.log(resp.data.fields.length);

        this.setState({
          form: resp.data,
          validId: true,
          answers: new Array(resp.data.fields.length).fill().map(function () {
    ...
    if (this.state.time !== prevState.time && this.state.form) {
      ...
      if (curr < start)
        ...
        setTimeout(async () => {
          ...
          let resp = await Axios.post("/checkForm", {
            id: this.state.id,
      ...
      <div className="form-container">
        ...
        >
          <Paragraph margin="default">
            Are you sure you want to submit? This process cannot be reverted.
          </Paragraph>
          <Button
            marginRight={16}
            appearance="primary"
        ...
        {form.fields && isActive && (
          <form onSubmit={this.handleSubmit} id="test-form">
            <Question
